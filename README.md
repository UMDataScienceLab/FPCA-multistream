...
The code performs a simulation where 50 units generates two streams of signals. One stream collects environmental information of each unit (e.g., voltage, temperature, etc.) while the other is the target stream which we want to extrapolate (e.g., degradation). Two distinct environments, Environment I (45 units) and II (5 units), are considered. The in-service unit is operated under Environment II in which less units are involved, resulting in a more challenging task.
